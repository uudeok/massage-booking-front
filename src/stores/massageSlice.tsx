...
export const massageSlice = createSlice({
  ...
  reducers: {
    ...
    getMassageTime(state, action) {
      state.detail = state.detail.filter(
        (masg) => masg.massageId === action.payload
      );
    },
  },
});
...
export const getMassageTimeList = (state: RootState) => state.massage.detail;
export const { getMassageDetail, getMassageTime } = massageSlice.actions;
